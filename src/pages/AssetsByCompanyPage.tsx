...
import type { DashboardCompanyCount } from '../types';
...
export default function AssetsByCompanyPage() {
  const { activeCompanyId, setActiveCompanyId } = useAuth();
  const navigate = useNavigate();
  const [rows, setRows] = useState<DashboardCompanyCount[] | null>(null);
  ...
  useEffect(() => {
    ...
    dashboardApi.getSummary(activeCompanyId ?? undefined, 1, controller.signal)
      .then((r) => { setRows(r.data.companyCounts); setLoadedAt(new Date()); })
      .catch((err) => {
  ...
  async function handleExport(format: 'excel' | 'pdf') {
    setExporting(true);
    try {
      await reportsApi.downloadCompaniesBreakdown({ format, companyID: activeCompanyId ?? -1 });
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Detailed breakdown of assets by company"
      ...
      <div className="px-4 sm:px-8 pt-3 pb-3 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        ...
        <MetricCard
          label="Top Company"
          value={loading || !top ? '—' : `${top.companyAbbreviation} (${topPct.toFixed(1)}%)`}
          accent="success"
        />
            ...
            <table className="w-full text-[13px]">
              ...
              <tbody className="divide-y divide-pearl-100">
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="text-center text-ink-300 py-8">No companies to show.</td></tr>
                ) : (
                  pageRows.map((c, i) => {
                    ...
                    return (
                      <tr
                        ...
                        onClick={() => { setActiveCompanyId(c.companyID); navigate('/assets'); }}
                      >
                        <td className="px-4 py-2.5 text-ink-300">{(pageNumber - 1) * pageSize + i + 1}</td>
                        <td className="px-4 py-2.5">
                          <div className="font-semibold text-ink-800">{c.companyAbbreviation}</div>
