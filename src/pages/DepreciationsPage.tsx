  ...
  return (
    <div className="px-8 py-6">
      ...
      <div className="flex gap-3 items-center bg-white p-4 rounded-lg shadow-[0_2px_8px_rgba(0,0,0,0.06)] mb-4">
        ...
        />
            <button onClick={runDepreciation} disabled={companyId == null} className="bg-[#9a7c4b] text-white border-none px-4 py-2 rounded-md text-[13px] font-semibold cursor-pointer hover:bg-[#7d6339] transition-colors">Run</button>
            <button onClick={deleteLast} disabled={companyId == null} className="bg-[#c0392b] text-white border-none px-4 py-2 rounded-md text-[13px] font-semibold cursor-pointer hover:bg-[#a93226] transition-colors">Delete Last</button>
          </>
        )}
      </div>

      {/* ── everything else only renders once a company is picked ── */}
      {companyId == null ? (
        <div className="flex flex-col items-center justify-center mt-24 text-center">
          <p className="text-[15px] font-semibold text-ink-600">Select a company</p>
          <p className="text-[13px] text-ink-300 mt-1">Choose a company above to manage its depreciation.</p>
        </div>
      ) : (
        <>
          {/* Info panel */}
